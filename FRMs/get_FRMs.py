...
if separate_silence:
    ...
    silence_cochleagram, _ = get_cochleagram(silence, fs, bin_size, n_F, min_F, max_F, 'spec_power', padding_bins, 
                                             zero_padding)
...
dsr = np.zeros((len(unique_best_lambdas), n_HUs, len(frequencies), len(stim_intensity)))
...
for lamb_idx in range(len(unique_best_lambdas)):
    ...
    if separate_silence:
        ...
        silence_h_traces = silence_traces['h']
        silence_h_win = silence_h_traces[:, win_left:win_right] # same window as tone
    ...
    for f_idx in range(len(frequencies)):
        for dB_idx in range(len(stim_intensity)):
            ...
            if not separate_silence:
                silence_h_win = h_traces[:, silence_win_left:silence_win_right]
            tone_h_win = h_traces[:, win_left:win_right]
            
            if use_whole_tone:
                dsr[lamb_idx, :, f_idx, dB_idx] = np.nanmean(tone_h_win - silence_h_win, 1) # driven spike rate
            else:
                silence_mean = np.expand_dims(np.nanmean(silence_h_win, 1), 1)
                if resp_diff_mode == 'square':
                    dsr[lamb_idx, :, f_idx, dB_idx] = np.nanmean(np.square(tone_h_win - silence_mean), 1)
                elif resp_diff_mode == 'abs':
                    dsr[lamb_idx, :, f_idx, dB_idx] = np.nanmean(np.abs(tone_h_win - silence_mean), 1)

max_sr = np.nanmax(dsr, (2, 3))
#max_sr = np.ones_like(dsr)
FRMs = np.zeros_like(dsr)
for lamb_idx in range(np.size(FRMs, 0)):
    for HU in range(np.size(FRMs, 1)):
        FRMs[lamb_idx, HU] = dsr[lamb_idx, HU]/max_sr[lamb_idx, HU]
