...
meta_ID = '.meta.json' # session metadata file ID
...
stim_ID = '.stim.h5' # stimuli file ID
...
for session in sessions:
    ...
    for i, j in enumerate(name_list):
        if j == 'PreStimSilence':
            start_times.append(float(data[i]['start'])) # list of end times of pre-stim silence
            end_times.append(float(data[i]['end'])) # list of start times of post-stim silence
    
    times = list(zip(start_times, end_times)) # list of tuples of start and end times for each stimulus
    
    ###########
    data_file = sess_dir + sess_dir_spec[0] + data_ID # full path to responses file
    stim_file = sess_dir + sess_dir_spec[0] + stim_ID # full path to stimuli file
    
    # only consider sessions where the same 306 stimuli were played; some sessions contain 307, where the 18 multi-repeat
    # stimuli are the same as in the sessions that contain 306. However, these sessions only have 2 single-repeat stimuli in
    # common with the 306 sessions, and were therefore ignored in all further analyses on NS2_include_single; interestingly
    # the stimuli that are contained in 307-stimuli sessions but not in 306-stimuli sessions are found elsewhere in the data
    # repository
    if len(unique_stim_occ) == 306:
        neuron_IDs.append(data_nID['cellid']) # list of neuron IDs from session
        spont_times.append(times) # append stimulus times to list containing all sessions
        
        data_in = h5py.File(data_file, 'r') # load response data from h5 file
        # load stimuli from h5 file (contains stimuli IDs and pre-processed cochleagrams, unused)
        stim_in = h5py.File(stim_file, 'r')
        ...
        neuron_spikes = []
        for nID in data_nID['cellid']: # loop over neurons in session
            print(nID)
...
NS2_prebuild_spontaneous = {'sessions': sessions, 'neuron_IDs': neuron_IDs, 'all_sess_resps': all_sess_resps, 
                            'silence_times': spont_times}

# save pre-built dataset file
'''f = open('NS2_prebuild_spontaneous.pckl', 'wb')
pickle.dump(NS2_prebuild_spontaneous, f)
f.close()'''